...
update_analyzers = function(ArStr) {
  ...
  analyzers[3].id='ar_stems_normalized_phonetic'
  analyzers[4].id='ar_root_normalized_phonetic'
  if (ArStr=="ArabicNoor") {
    analyzers[1].id='ar_stems_noor'
    analyzers[2].id='ar_root_noor'
    analyzers[3].id='ar_stems_normalized_phonetic_noor'
    analyzers[4].id='ar_root_normalized_phonetic_noor'
  }
}
...
aggregates = [                                      //order is important
      ...
      {id:'s_English_Corpus_phrases', name:"Significant English Corpus Phrases", lang:"EnglishCorpus"},
      {id:'s_English_Corpus', name:"Significant Words from English Corpus", lang:"Corpus (English)"},
      {id:'s_English_Corpus_to_Arabic', name:"Significant Arabic words from English Corpus", lang:"Arabic"},
...
genFilterDSL = function(query){
  ...
  query.map((q)=>{
    ...
    } else if (q[1].search(/^[\d]+-[\d]+$/)!="-1") {      //Filter ranges by number start and end
      let range={}
      qvArr = q[1].split(/-/)
      qvS = parseInt(qvArr[0])                    //Query Value Start
