...
genFilterDSL = function(query,fields){
  ...
  query.map((q)=>{
    ...
    if (q[1].search(/^[<>][>=]?[\d]+$/)!="-1") {      //Filter ranges by number
      ...
      let fld=q[0]
      if (Number.isInteger(Number(fld))){
        fld = "a"                   // for Ayah
        let term={}
      ...
      let fld=q[0]
      if (Number.isInteger(Number(fld))){               //chapter match
      ...
      if (Number.isInteger(Number(fld))){               //chapter match
        fields.push("s")            // Permanent addition for chapter refence
        let term = {}
        term["CR_all"]=q[0]+"\:"+q[1].replace(/~/g,'')              // for Surah
      ...
      if (q[0]=="ayah") {        // special case where ayahs are specified, not filtered
        ...
        simple_query_string={
          fields:[(q[0]=="" || q[0]=="*")?"*.*_original*":q[0]],default_operator: "and",
          query: q[1].replace(/\"/g, '""')
          }
