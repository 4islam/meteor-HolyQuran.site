globalLimit = 5    //ui limit
mongoCacheCount = 15
defaultConfStr = "0X152353"
...
aggregates = [                                      //order is important
      ...
      {id:'s_UrduTSN_phrases', name:"تفسیر صغیر کی اہم اردو عبارتیں", lang:"Urdu", esField:"UrduTSN"},
      {id:'s_UrduTSN', name:"تفسیر صغیر کے اہم اردو الفاظ", lang:"Urdu", esField:"UrduTSN"},
...
limit = globalLimit;        //search result limit
...
genFilterDSL = function(query,fields){
  ...
  query.map((q)=>{
    ...
    if (q[1].search(/^[<>][>=]?[\d]+$/)!="-1") {      //Filter ranges by number
      ...
      if (q[0].search(/^[\-]/) !="-1" ) {
        simple_query_string.default_operator="or"
        ...
        //   let match_phrase = {}                                  // because all layers must have a match, only works with one layer
      ...
      if (q[0]=="ayah") {        // special case where ayahs are specified, not filtered
        ...
        simple_query_string={
          fields:fld,
          default_operator: "or",
          // query: q[1].replace(/\"/g, '""')                   // Removed to handle multi-word global string search
          // query: "\""+q[1]+"\""
          query: q[1]
          }
          if (simple_query_string.query.indexOf(' ') >= 0) {    // Added to handle multi-word global string search
            simple_query_string.query="\""+simple_query_string.query+"\""
          }
        must.push({"simple_query_string":simple_query_string})
      } else {
...
hexEncodeJSON = function (jsonObj) {
  let finalHexStr=""
  jsonObj.map((i,x)=>{
      if (i.state) {
      	// console.log(x, i.id, i.name)
          	Object.keys(i).map((j,y)=>{
                  if (j=="options") {
          		// console.log("\t",x,":",j, i[j] , j.length)
                  let bstr=""
                  i[j].map((k,z)=>{
                      if (k.state) {
                          bstr+="1"
              ...
              let finalbstr=(x >>> 0).toString(2).padStart(6,"0")+bstr.padStart(6,"0")
              finalHexStr+=parseInt(finalbstr, 2).toString(16).padStart(3,"0")
              }
      	})
      }
  })
...
encodingTab =
  ...
  //Soure: https://codeheir.com/2022/07/02/coding-a-base64-encoder-in-javascript/

ub64Encode = function (jsonObj) {     //URL Safe Base 64
  let finalB64Str=""
  jsonObj.map((i,x)=>{
      if (i.state) {
      	// console.log(x, i.id, i.name)
          	Object.keys(i).map((j,y)=>{
                  ...
                  finalB64Str+=encodingTab.charAt(x)+encodingTab.charAt(parseInt(bstr, 2))
              }
      	})
      }
  })
...
ub64Decode = function (encodedStr) {
  let returnArray=[]
