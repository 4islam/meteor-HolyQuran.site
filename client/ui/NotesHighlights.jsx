    ...
    return (
        <div className={this.props.Type+" reference"}>
            {(this.props.highlights)?Object.keys(this.props.highlights).map(function(k,i) {
                ...
                if (k.search(re)!=-1) {
                   return (
                      <ul key={this.props.ayah+"-"+k+"."+i} className={"fragments " + k.replace(/\.|_/g,' ')}>
                        {
                          this.props.highlights[k].map((l,j)=>(
                            <span key={this.props.ayah+"-"+k+"."+i+"."+j} dangerouslySetInnerHTML={{__html: " ... " + this.props.highlights[k][j].replace(/(\u06E3|\u06E8)/,' $1') + " "}}/>
                          ))
                        }
                      ...</ul>
                    )
