            ...
            {(this.props.highlights)?Object.keys(this.props.highlights).map(function(k) {
                ...
                return (k.search(re)!=-1?
                  <div key={k} className={"highlights noselect " + k.replace(/\.|_/g,' ')}
                    dangerouslySetInnerHTML={{__html: this.props.highlights[k][0].replace(/(\u06E3|\u06E8)/,' $1').replace(/(\[[^\]]+\])/g,'<sup>$1</sup>')}}/>
                  :'');
              }.bind(this))
            :''}
            <div className={'highlights noselect ' + this.props.Type + ' base container-fluid Verse'}
              dangerouslySetInnerHTML={{__html: this.props.base.replace(/(\u06E3|\u06E8)/,' $1').replace(/(\[[^\]]+\])/g,'<sup>$1</sup>')}}/>
